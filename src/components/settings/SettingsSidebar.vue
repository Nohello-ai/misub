...
function isLocked(tab) {
  return tab.adminOnly === true && !isAdmin.value;
}
</script>

<template>
  <nav class="grid grid-cols-2 gap-2 p-1 md:flex md:flex-col md:space-y-1 md:gap-0 md:p-0">
    <button v-for="tab in tabs" :key="tab.id" @click="isLocked(tab) ? null : emit('update:activeTab', tab.id)"
      ...
      :class="[
        activeTab === tab.id && !isLocked(tab)
          ? 'bg-primary-600 text-white shadow-sm shadow-primary-500/25 border border-white/10'
          : isLocked(tab)
            ? 'text-gray-400 dark:text-gray-600 opacity-50 cursor-not-allowed select-none'
      ...
      <span class="flex items-center gap-1">
        {{ t(tab.labelKey) }}
        <svg v-if="isLocked(tab)" class="h-3 w-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
