...
export const useSessionStore = defineStore('session', () => {
  ...
  // 角色控制:从 /api/me 拉取当前用户角色(admin/user)
  const userRole = ref('user');
  async function fetchMe() {
    try {
      const d = await api.get('/api/me');
      userRole.value = d?.user?.role || 'user';
    } catch { /* 拉取失败保持默认 */ }
  }
  const isAdmin = computed(() => userRole.value === 'admin');

  return { sessionState, initialData, publicConfig, subscriptionConfig, securityWarning, userRole, isAdmin, fetchMe, checkSession, login, logout };
});
