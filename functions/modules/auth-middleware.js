...
export async function getAuthSessionDiagnostic(request, env) {
    ...
    // token 数据格式:timestamp[:userID[:username]](向后兼容纯 timestamp)
    const parts = String(verifiedData).split(':');
...
export async function handleLogin(request, env) {
    ...
    try {
        ...
        if (base) {
            ...
            } catch (_) { /* v2 不可达,走本地密码兜底 */ }
        }

        if (!userID) {
            // 兜底:本地管理员密码(默认 admin,或 KV SYSTEM_ADMIN_PASSWORD)
            const inputPassword = normalizeSecret(password);
            const currentPassword = normalizeSecret(await getAdminPassword(env));
            if (inputPassword === currentPassword) {
                const localAdminUUID = String(env?.ADMIN_UUID || '').trim();
                userID = localAdminUUID || null;
            }
        }

        if (userID) {
            const secret = await getCookieSecret(env);
            const token = await createSignedToken(secret, `${Date.now()}:${userID}:${userName}`);
            const isSecure = request.url.startsWith('https');
            const cookieString = `${COOKIE_NAME}=${token}; Path=/; HttpOnly; ${isSecure ? 'Secure;' : ''} SameSite=Lax; Max-Age=${SESSION_DURATION / 1000}`;
            return new Response(JSON.stringify({ success: true, user: { userID, username: userName } }), {
                headers: {
