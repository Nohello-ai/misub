...
/**
 * 订阅响应附加流量头:直接从用户管理(v2)接口拉取配额/用量,客户端显示剩余流量
 * 单账号模式:统一用管理员账号的配额/用量
 */
async function attachTrafficHeader(response, env) {
    if (!response || !response.headers || response.headers.get('subscription-userinfo')) return response;
    const base = String(env?.ADMIN_API_BASE || '').replace(/\/+$/, '');
    const adminToken = env?.ADMIN_API_TOKEN || '';
    const adminUUID = String(env?.ADMIN_UUID || '').trim() || 'a5aea414-bd01-48f1-9c82-1553c57b4033';
    if (!base || !adminToken) return response;
    try {
        const res = await fetch(`${base}/api/admin/users/${adminUUID}`, {
        ...
        if (u) {
            const headers = new Headers(response.headers);
            headers.set('subscription-userinfo', `upload=${Number(u.upload || 0)}; download=${Number(u.download || 0)}; total=${Number(u.quota || 0)}; expire=4102329600`);
    ...
    try {
        const handleRequest = async () => {
            ...
            if (url.pathname.startsWith('/api/')) {
                ...
                // MiSub 订阅路由
                return await attachTrafficHeader(await handleMisubRequest(context), env);
